...
import { Container, Nav, Image, ListGroup, Card, Button, Dropdown, DropdownButton, Navbar, Col, Media, Row } from 'react-bootstrap';
...
export default function Post({ query, Post, Categories, FeaturedPost, TopFive, LatestPosts}) {
  ...
  const handleSelect = (e) => {
    setDropTitle(e);
    if (e == "Top News") {
      const fetchPromise = fetch(`http://dcoders.rs:1337/posts?_start=0&&_limit=10&&categories.slug=${query.post}&&_sort=views:DESC`);
...
Post.getInitialProps = async ({ query }) => {
    if (
      query.post == 'medicine' || 
      query.post == 'legal-issues' || 
      query.post == 'hot-products' || 
      query.post == 'customers'
    ) {
      const Post = null,
            Categories = await CategoriesController.getCategories(),
            FeaturedPost = await PostsController.getFeaturedPost(query.post),
            TopFive = await PostsController.getTopFive(query.post),
            LatestPosts = await PostsController.latestPosts(query.post, null, 1);
      return { query, Categories, FeaturedPost, TopFive, LatestPosts, Post }
    } else {
      const Post = await PostsController.getSinglePost(query.post),
            Categories = await CategoriesController.getCategories(),
            FeaturedPost = await PostsController.getFeaturedPost(null),
            TopFive = await PostsController.getTopFive(null),
            LatestPosts = await PostsController.latestPosts(null, null, 1);
      return { query, Categories, FeaturedPost, TopFive, LatestPosts, Post }
    }
}